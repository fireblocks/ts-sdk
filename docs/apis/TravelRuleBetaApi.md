...
[**travelRuleApiControllerUpdateVasp**](#travelRuleApiControllerUpdateVasp) | **PUT** /screening/travel_rule/vasp/update | Add jsonDidKey to VASP details
...
import { readFileSync } from 'fs';
import { Fireblocks, BasePath } from '@fireblocks/ts-sdk';
import type { FireblocksResponse, TravelRuleBetaApiTravelRuleApiControllerUpdateVaspRequest, TravelRuleUpdateVASPDetails } from '@fireblocks/ts-sdk';
...
let body: TravelRuleBetaApiTravelRuleApiControllerUpdateVaspRequest = {
...
fireblocks.travelRuleBeta.travelRuleApiControllerUpdateVasp(body).then((res: FireblocksResponse<TravelRuleUpdateVASPDetails>) => {
  console.log('API called successfully. Returned data: ' + JSON.stringify(res, null, 2));
}).catch((error:any) => console.error(error));
```
