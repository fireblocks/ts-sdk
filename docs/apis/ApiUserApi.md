...
[**issueApiUserPairingToken**](#issueApiUserPairingToken) | **POST** /management/api_users/{userId}/pairing_token | Issue API user pairing token
...
> IssueApiUserPairingTokenResponse issueApiUserPairingToken()
...
import { readFileSync } from 'fs';
import { Fireblocks, BasePath } from '@fireblocks/ts-sdk';
import type { FireblocksResponse, ApiUserApiIssueApiUserPairingTokenRequest, IssueApiUserPairingTokenResponse } from '@fireblocks/ts-sdk';
...
let body: ApiUserApiIssueApiUserPairingTokenRequest = {
...
fireblocks.apiUser.issueApiUserPairingToken(body).then((res: FireblocksResponse<IssueApiUserPairingTokenResponse>) => {
  console.log('API called successfully. Returned data: ' + JSON.stringify(res, null, 2));
}).catch((error:any) => console.error(error));
```
...
**[IssueApiUserPairingTokenResponse](../models/IssueApiUserPairingTokenResponse.md)**
