...
import { readFileSync } from 'fs';
import { Fireblocks, BasePath } from '@fireblocks/ts-sdk';
import type { FireblocksResponse, ApprovalsBetaApiRejectApprovalRequest } from '@fireblocks/ts-sdk';
...
let body: ApprovalsBetaApiRejectApprovalRequest = {
  // string | The approval request ID.
  requestId: 18055,
...
fireblocks.approvalsBeta.rejectApproval(body).then((res: FireblocksResponse<any>) => {
  console.log('API called successfully. Returned data: ' + JSON.stringify(res, null, 2));
}).catch((error:any) => console.error(error));
```
