...
export const TransactionsApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @param {string} [before] Unix timestamp in milliseconds. Returns only transactions created before the specified date. Provides an explicit end time. If not provided, default value will be applied, and may change over time.  The current default value is the past 90 days. 
         * @param {string} [after] Unix timestamp in milliseconds. Returns only transactions created after the specified date. Provides an explicit start time. If not provided, default value will be applied, and may change over time.  The current default value is the past 90 days. 
         * @param {string} [status] You can filter by one of the statuses.
         * @param {GetTransactionsOrderByEnum} [orderBy] The field to order the results by.  **Note:** Ordering by a field that is not &#x60;createdAt&#x60; may result in transactions that receive updates as you request the next or previous pages of results, resulting in missing those transactions. 
...
export interface TransactionsApiGetTransactionsRequest {
    ...
    /**
     * The field to order the results by.  **Note:** Ordering by a field that is not &#x60;createdAt&#x60; may result in transactions that receive updates as you request the next or previous pages of results, resulting in missing those transactions. 
